...
class TemperatureMachine(StateMachine):
    ...
    # Calling getFahrenheit everytime the temp was needed lead to issues getting
    # data from the AHT20 sensor. This caused the program to crash and other issues. The cause of this issue
    # comes from the getFahrenheit method being called from multiple threads. 
    # To fix this issue I added the currTemp variable which holds
    #  the temperature and is updated every second. This also means that the same temperature is used throughout the program.
    currTemp = None
    ...
    def updateLights(self):
        ...
        # Use the currTemp variable to set temp in function to limit the number of times
        # the AHT20 sensor is accessed.
        temp = floor(self.currTemp)
        ...
        # If the in heat state and temp is less than setPoint have light pulse
        # other wise have light on.
        ...
        # If in cool state and temp is greather setPoint pulse blue light 
        # other wise light on.
    ...
    def setupSerialOutput(self):
        ##
        # Build UART output string with state, temp and setPoint.
        output = f"{self.current_state.id},{floor(self.currTemp)},{self.setPoint}"
    ...
    def manageMyDisplay(self):
        ...
        while not self.endDisplay:
            ...
            # The currTemp varible is updated every second.
            self.currTemp = self.getFahrenheit()
    
            ## Setup display line 1

            ##
            # Build string for the first line of the LCD screen
            lcd_line_1 = current_time.strftime('%b %d  %H:%M:%S\n')
    
            ## Setup Display Line 2
            if(altCounter < 6):
                ##
                ## Build string for first message of second line of LCD screen
                ## using the currTemp varible rather than calling getFarhenheit function.
                lcd_line_2 = f"TEMP: {floor(self.currTemp)}F"
    
                altCounter = altCounter + 1
            else:
                ##
                ## Build string for second message of second line of LCD screen.
                lcd_line_2 = f"{self.current_state.id.upper()}: Set To {self.setPoint}"
            ...
            if((counter % 30) == 0):
                ##
                ## Transmit data using UART sending output data.
